<?php

namespace Loco\Tests\Http\Generated;

use Loco\Tests\Http\ApiClientTestCase;
use Loco\Tests\Http\Model\MockRequest;
use Loco\Tests\Http\Model\MockResponse;

/**
 * Auto-generated Loco API command test.
 *
 * DO NOT EDIT
 *
 * @group built
 */
class GetLocaleProgressCommandTest extends ApiClientTestCase
{

    /**
     * Get more detailed translation progress for a project locale
     */
    public function testGetLocaleProgressCommandSuccess()
    {
        $service = $this->getServiceDescription();
        $query = new MockRequest('getLocaleProgress', $service);
        $model = new MockResponse('object', $service);
    
        $client = $this->getClientWithMockedResponse(
            [ 'base_uri' => 'https://example.com/api' ],
            $model->toArray()
        );

        $result = $client->getLocaleProgress($query->toArray());
        $this->assertInstanceOf($model->getResponseClass(), $result, 'Bad class for "object" model');
    }
}
